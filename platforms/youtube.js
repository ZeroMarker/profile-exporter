const PlatformYouTube = {
    ...
    return fetchAllPages(
      async ({ cursor }) => {
        ...
        if (!match) {
          ...
          if (!altMatch) return { items: [], nextCursor: null };
  ...
  parseSubscriptionsData(jsonStr) {
    try {
      ...
      const tab = tabs.find((t) => t.tabRenderer?.title === "Channels");
      const items =
        tab?.tabRenderer?.content?.richGridRenderer?.contents ||
        tab?.content?.sectionListRenderer?.contents?.[0]?.itemSectionRenderer?.contents?.[0]
      ...
      for (const item of items) {
        const channel = item.richItemRenderer?.content?.channelRenderer;
        if (channel) {
          channels.push({
        ...
        const cont = item.richItemRenderer?.content?.continuationItemRenderer?.continuationEndpoint?.continuationCommand?.token;
